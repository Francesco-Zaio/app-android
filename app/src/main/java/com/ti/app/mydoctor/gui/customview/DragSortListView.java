...
public class DragSortListView extends ListView {
    ...
    /**
     * Determines the startOperation of the upward drag-scroll region
    ...
    /**
     * Determines the startOperation of the downward drag-scroll region
    ...
    private class RemoveAnimator extends SmoothAnimator {
        ...
        public void onUpdate(float frac, float smoothFrac) {
            ...
            if (item != null) {
            	/*if(DragSortController.getDirectionRemove() == Direction.LEFT) {
            		...
            		if (mRemoveListener != null) {
                        mRemoveListener.itemChangeLayout(mFirstPos, View.VISIBLE);
                    }
            	}  */  
    ...
    private void doRemoveItem() {
    	...
    	doRemoveItemRequest(mSrcPos - getHeaderViewsCount());
    }
    
    public void removeItemFromList(int which) {
        doRemoveItem(which);    	
    }
    
    /**
     * Request to removes dragged item from the list. Calls RemoveListener.
     */
    private void doRemoveItemRequest(int which) {
    	
    	mDragState = DRAGGING;
    	
    	/*if ((mDragFlags & DRAG_NEG_X) == 0) {
    		int I = 0;
    		
    		// end it
            if (mRemoveListener != null) {
                mRemoveListener.removingRequest(which, DragSortController.getDirectionRemove());
            }
    	} else {
    		mDragState = STOPPED;
    	}*/
    ...
    private void continueDrag(int x, int y) {
        ...
        if (minY > mLastY && minY > mDownScrollStartY && currentScrollDir != DragScroller.DOWN) {
            // dragged down, it is below the down scroll startOperation and it is not
            // scrolling up
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        ...
        //mDragFlags |= DRAG_NEG_X;
        //DragSortController.setDirectionBlockRemove(Direction.RIGHT);
    ...
    public interface RemoveListener {
    	void itemChangeLayout(int which, int mode);
    	void removingRequest(int which, Direction direction);
    ...
    public void moveCheckState(int from, int to) {
        ...
        // startOperation and end of the "runs" of checked items, and then moving the
    ...
    private static int buildRunList(SparseBooleanArray cip, int rangeStart,
        ...
        if (currentRunEnd == rangeEnd) {
            ...
            // checking if the startOperation equals the end position.
    ...
    public interface DragScrollProfile {
         ...
         * @param t Time (in milliseconds) since startOperation of scroll (handy if you
