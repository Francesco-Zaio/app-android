...
import com.ti.app.mydoctor.core.btmodule.events.BTSocketEvent;
import com.ti.app.mydoctor.core.btmodule.events.BTSocketEventListener;
...
public class BTSocket implements BTSocketReadEventListener, BTSocketWriteEventListener {
	...
	private Runnable connectInsecureRunnableUUID = new Runnable() {
		...
		public void run() {
			try {      
	    		...
	    		//mmSocket = btDevice.createRfcommSocketToServiceRecord(SERIALPORT);
				Method m = btDevice.getClass().getMethod("createInsecureRfcommSocketToServiceRecord", UUID.class);
	...
	private Runnable connectInsecureRunnable = new Runnable() {
		...
		public void run() {
			try {      
	    		//mmSocket = btDevice.createRfcommSocketToServiceRecord(SERIALPORT);
				Method m = btDevice.getClass().getMethod("createInsecureRfcommSocket", int.class);
				mmSocket = (BluetoothSocket) m.invoke(btDevice, 1);							
				mmSocket.connect();
	            //Prima fase viene aperto lo stream in lettura        
	            openReaderStream();            
